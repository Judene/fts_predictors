...
if __name__ == "__main__":
    ...
    mlp_predictions, mlp_val_score, mlp_error = wf_model.train_and_predict(gold_etf_data, gold_etf_data)
    print("MLP Walk Forward")
    print(np.mean(mlp_val_score))
    print(np.mean(mlp_error ** 2.0))

    # Create a plot of our errors through time
    mlp_error = mlp_error ** 2.0
    mlp_error = mlp_error.set_index(pd.to_datetime(mlp_error.index))
    mlp_error.plot()
    plt.show()
    plt.close()
