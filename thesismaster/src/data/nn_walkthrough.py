...
class WalkForwardPredictor:
    ...
    def __init__(self, model, start_date="2015-04-27", end_date="2021-05-24", input_pct_change=1,
...
if __name__ == "__main__":
    ...
    #indexNames = zar_currency[ zar_currency['Vol.'] == '-' ].index
    #zar_currency.drop(indexNames , inplace=True)


    #Final Cleaning of dataset
    zar_currency = zar_currency.dropna()
    zar_currency = exploration(zar_currency)
    
    import sklearn.preprocessing    
    def normalize_data(exchange):
        min_max_scaler = sklearn.preprocessing.MinMaxScaler()
        zar_currency['Price'] = min_max_scaler.fit_transform(zar_currency.Price.values.reshape(-1,1))
        zar_currency['Open'] = min_max_scaler.fit_transform(zar_currency.Open.values.reshape(-1,1))
        zar_currency['High'] = min_max_scaler.fit_transform(zar_currency.High.values.reshape(-1,1))
        zar_currency['Low'] = min_max_scaler.fit_transform(zar_currency.Low.values.reshape(-1,1))
        return exchange
    zar_currency = normalize_data(zar_currency)

    input_data = zar_currency.drop(['Price','Change','Vol.'], axis = 1)
    zar_currency = zar_currency.drop(['Price', 'Open', 'High', 'Low', 'Vol.'], axis = 1)
    
    # Initiate our model
    model = WalkForwardPredictor(model=neuralnetwork, start_date="2015-04-27", end_date="2021-05-24",
