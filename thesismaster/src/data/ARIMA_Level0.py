...
class WalkForwardPredictor:
    ...
    def __init__(self, model, start_date="2015-05-01", end_date="2021-02-01", input_pct_change=1,
...
if __name__ == "__main__":
    ...
    input_data = zar_currency.drop(['Open','High','Low','Vol.','Price'], axis = 1)
    zar_currency = zar_currency.drop(['Open','High','Low','Vol.','Price'], axis = 1)
    
    # Initiate our model
    model = WalkForwardPredictor(model=ARIMA(), start_date="2015-05-01", end_date="2021-02-01",
    ...
    tree_error.plot()
    plt.title('MSE Gold % Change: Level 0 ARIMA')
    plt.xlabel('Time')
    plt.ylabel('MSE')
    plt.show()
    plt.close()
