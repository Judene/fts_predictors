...
class WalkForwardPredictor:
    ...
    def train_and_predict(self, input_data, output_data, datum):
        ...
        input_data = scaler1.fit_transform(input_data)
        scaler2 = MinMaxScaler(feature_range=(-1, 1))
        output_data = scaler2.fit_transform(output_data)
        
        input_data = pd.DataFrame(input_data, columns=['var1(t-2)', 'var1(t-1)'])
        input_data['Date'] = datum
        input_data = input_data.set_index('Date')
        input_data = input_data.dropna()
        
        output_data = pd.DataFrame(output_data, columns=['var1(t)'])
        output_data['Date'] = datum
        ...
        pred_df = pd.DataFrame(np.column_stack([predictions, np.squeeze(output_data)]))
        pred_df.columns = ["PRED", "TRUE"]
        pred_df.to_csv('C:/Users/ELNA SIMONIS/Documents/Results/Bond_LRWF.csv')
...
if __name__ == "__main__":
    ...
    # Initiate our model
    model = WalkForwardPredictor(model=LinearRegression(), start_date="2001-01-08", end_date="2021-07-30",
    ...
    tree_predictions, tree_val_score, tree_error = model.train_and_predict(input_data, output_data, datum)
    ...
    print("AME" ,np.mean(np.sum(abs(tree_error))))
    
    d = abs(tree_error)
    d.dtypes
    d["error"] = pd.to_numeric(d["error"])
    x = d.mean(axis=0)
    print(x)
    ...
    plt.plot(tree_error, color = 'blue')
