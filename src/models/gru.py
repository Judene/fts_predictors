...
class GRURecurrentNN(FTSModel):
    ...
    def __init__(self, name: str, num_inputs: int, num_outputs: int, *args, **kwargs):
        ...
        for i in range(len(self.gru_layers)):
            ...
            return_sequences = i != (len(self.gru_layers) - 1)

            # --- Add a dropout layer on the inputs ---

            layers.append(
                Dropout(
                    rate=self.gru_layers[i][3]
                )(layers[-1])
            )
            ...
            if self.use_cudnn_layers:
                layers.append(
                    CuDNNGRU(
                        units=self.gru_layers[i][0],
                        stateful=self.stateful_training,
                        return_sequences=return_sequences,
                        kernel_regularizer=l2(self.gru_layers[i][1]),
                        recurrent_regularizer=l2(self.gru_layers[i][2])
                    )(layers[-1])
                )
            else:
                layers.append(
                    GRU(
                        ...
                        stateful=self.stateful_training,
                        return_sequences=return_sequences,
                        recurrent_dropout=self.gru_layers[i][4],
                        kernel_regularizer=l2(self.gru_layers[i][1]),
                        recurrent_regularizer=l2(self.gru_layers[i][2])
                    )(layers[-1])
                )
