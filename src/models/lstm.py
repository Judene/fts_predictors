...
from tensorflow.keras.layers import Dense, Dropout, Input, GaussianDropout, LSTM, CuDNNLSTM, Reshape
...
class LSTM(FTSModel):
    """
    An implementation of a LSTM (Long short term memory).
    """
    ...
    def __init__(self, name: str, num_inputs: int, num_outputs: int, *args, **kwargs):
        ...
        for i in range(len(self.lstm_layers)):
            ...
            if self.use_cudnn_layers:
                layers.append(
                    CuDNNLSTM(
                        units=self.lstm_layers[i][0],
                        stateful=self.stateful_training,
                        return_sequences=return_sequences,
                        kernel_regularizer=l2(self.lstm_layers[i][1]),
                        recurrent_regularizer=l2(self.lstm_layers[i][2])
                    )(layers[-1])
                )
            else:
                layers.append(
                    LSTM(
                        ...
                        stateful=self.stateful_training,
                        return_sequences=return_sequences,
                        recurrent_dropout=self.lstm_layers[i][4],
                        kernel_regularizer=l2(self.lstm_layers[i][1]),
                        recurrent_regularizer=l2(self.lstm_layers[i][2])
                    )(layers[-1])
                )
